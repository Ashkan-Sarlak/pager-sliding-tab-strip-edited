...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private Rect tabIndicatorBounds;
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		tabIndicator = getResources().getDrawable(R.drawable.tab_indicator);
		tabIndicatorBounds = new Rect();
		tabIndicatorWidth = tabIndicator.getIntrinsicWidth();
		tabIndicatorHeight = tabIndicator.getIntrinsicHeight();
	...
	private void replaceIconTab(final int position, int resId) {
		View tab = tabsContainer.getChildAt(position);
		tabsContainer.removeView(tab);
	...
	protected void onDraw(Canvas canvas) {
		...
		int tabMiddle = (int) ((lineLeft + lineRight) / 2);

		tabIndicatorBounds
				.set(tabMiddle - tabIndicatorWidth / 2, height
						- tabIndicatorHeight,
						tabMiddle + tabIndicatorWidth / 2, height);
		tabIndicator.setBounds(tabIndicatorBounds);
		...
		public void onPageSelected(int position) {
			...
			if (pager.getAdapter() instanceof IconTabProvider) {
				replaceIconTab(mCurrentPagePosition,
						((IconTabProvider) pager.getAdapter())
								.getPageIconResId(mCurrentPagePosition));

				replaceIconTab(position,
						((IconTabProvider) pager.getAdapter())
								.getPageSelectedIconResId(position));
